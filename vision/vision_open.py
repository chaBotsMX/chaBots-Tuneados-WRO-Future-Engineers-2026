...
# blob.code usa el bit 1 para el primer threshold y el bit 2 para el segundo.
LINE_THRESHOLDS = [
    (0, 79, -128, 20, -128, -7),  # ID 1: azul
    (0, 79, -128, 20, -128, -7),  # ID 2: placeholder naranja
]
...
def main():
    ...
    while True:
        ...
        # Si no se detecta ninguna línea no se transmite nada.
        if detected_blob is not None:
            ...
            img.draw_cross(
                (detected_blob.cx, detected_blob.cy),
                color=detected_color,
                thickness=2,
            )

            if PRINT_DIAGNOSTICS:
                print("line id:", detected_id)
